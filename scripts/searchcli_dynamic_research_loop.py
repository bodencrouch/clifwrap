...
def discover_accounts() -> list[Account]:
    proc = run(["searchcli", "auth", "list"])
    if proc.returncode != 0:
        raise RuntimeError((proc.stderr or proc.stdout).strip() or "searchcli auth list failed")
...
def default_account() -> str:
    proc = run(["searchcli", "auth", "default"])
...
def set_default(account: str) -> None:
    proc = run(["searchcli", "auth", "use", account])
    if proc.returncode != 0:
...
def print_results(stdout: str) -> None:
    ...
    print(f"Outcome: success. SearchCLI returned {len(results)} results.", flush=True)
    ...
    if answer:
        print("  SearchCLI direct answer:", flush=True)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run verbose SearchCLI searches until wrapper failover is observed.")
    ...
    print("Dynamic SearchCLI proof loop starting.", flush=True)
    print("No account label is hardcoded in this script; enabled account names were discovered from `searchcli auth list`.", flush=True)
    print(f"Discovered enabled accounts in wrapper order: {account_names(accounts)}", flush=True)
    print("The script will rotate the wrapper default through those discovered accounts until a real SearchCLI retry/failover is observed.", flush=True)
    ...
    while args.max_queries <= 0 or query_number < args.max_queries:
        ...
        command = ["searchcli", "--json", "search", topic]

        print(f"[{datetime.now().isoformat(timespec='seconds')}] Dynamic SearchCLI research query #{query_number}", flush=True)
