...
DEFAULT_SPEC = Path(__file__).with_name("scrapecli_requested_accounts.toml")
DEFAULT_CREDENTIALS = Path("~/.config/scrapecli/credentials.json").expanduser()
...
def _write_selected_spec(original: dict, accounts: list[dict]) -> Path:
    handle = tempfile.NamedTemporaryFile("w", suffix=".toml", prefix="clifwrap-scrapecli-", delete=False)
    with handle:
...
def _logout_current_credentials() -> None:
    env = os.environ.copy()
    env["CLIFWRAP_BYPASS"] = "1"
    subprocess.run(["scrapecli", "logout"], env=env, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Capture multiple ScrapeCLI logins into declarative account sources used by clifwrap account import-spec."
    )
    ...
    parser.add_argument("--credentials", default=str(DEFAULT_CREDENTIALS), help="ScrapeCLI credentials.json path to read after each login.")
    parser.add_argument("--method", choices=["browser", "manual"], default="browser", help="Upstream ScrapeCLI login method.")
    ...
    for account in accounts:
        ...
        if already_configured and not args.replace_existing:
            ...
            continue
        print(f"{label}: prepare the upstream ScrapeCLI login for this account.", flush=True)
        ...
        if status != 0 and not timed_out:
            print(f"{label}: scrapecli login exited with status {status}; stopping.", file=sys.stderr)
            return status
