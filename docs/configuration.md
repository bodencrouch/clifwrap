...
Provider behavior is declarative. Built-in metadata for `searchcli` and `scrapecli` lives in `src/clifwrap/providers.toml`; user config should normally define only accounts and local policy overrides.
...
Wrapper-managed auth aliases are separate from upstream auth commands. This lets commands such as `searchcli logins` or `scrapecli accounts` manage wrapper accounts while a bare canonical auth command can still pass through to the real CLI.
